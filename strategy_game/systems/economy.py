...
def compute_world_inflation(previous_prices: Dict[str, float], current_prices: Dict[str, float]) -> float:
    """Compute a global inflation signal from a stable core goods basket."""
    basket = ["food", "wood", "stone", "iron", "advanced_goods", "luxury"]
    changes = []
    for key in basket:
        prev = previous_prices.get(key, 0.0)
        if prev <= 0:
            continue
        changes.append((current_prices.get(key, prev) - prev) / prev)
    if not changes:
        return 0.0
    return max(-0.1, min(0.1, sum(changes) / len(changes)))
...
def apply_inflation_and_upkeep(
    empire: Empire,
    market: MarketState,
    military_upkeep: float,
    inflation_sensitivity: float,
    world_inflation: float,
) -> float:
    ...
    internal_push = money_supply * inflation_sensitivity / 10000
    empire.inflation = min(0.35, max(-0.05, empire.inflation * 0.75 + internal_push + world_inflation * 0.25))
    return total_upkeep
